...
### Examining GameObjects and components

When we select a GameObject in the Scene view or the Hierarchy panel, we can see the details of all its components in the Inspector panel.

Some components have **_fields_** where you can change **_values_**. Fields can take various forms, such as boxes you can type into, sliders, checkboxes, colour pickers. These fields are in fact variables, just like the variables we created in Scratch.

### Making our own components from scripts
...
A variable is a piece of information that we want to remember and use later. When we create a variable, we say that we are **_declaring_** it. When we change its value, we say that we are **_setting_** its value. When we find out its value, we say that we are **_getting_** its value.
...
* *name* - this variable can be called on a GameObject or any component. It is a string that contains the name displayed in the hierarchy for the GameObject in question.
...
private void PrintName(string nameToPrint)

{

	Debug.Log("PrintName has been called and the name is " + nameToPrint);
